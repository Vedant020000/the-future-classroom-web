...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.log("Assessment Creator API received:", body);

        // --- Input Validation ---
        const { type, topic, gradeLevel, numQuestions, objectives } = body;
        if (!type || !topic || !gradeLevel) {
            return NextResponse.json({ error: 'Bad Request', details: 'Missing required fields: type, topic, gradeLevel.' }, { status: 400 });
        }

        // --- Construct Prompt ---
        const prompt = `Generate an assessment based on the following details:
Assessment Type: ${type}
Topic/Subject Area: ${topic}
Grade Level: ${gradeLevel}
${numQuestions ? `Number of Questions/Items: ${numQuestions}` : ''}
${objectives ? `Learning Objectives Covered: ${objectives}` : ''}

Please generate the assessment content directly.`;

        // --- Call Gemini API ---
        let responseText = "Sorry, I couldn't generate the assessment.";
        try {
            console.log("Sending Assessment Creator prompt to Gemini...");
            const result = await model.generateContent(prompt);
            responseText = result.response.text();
            console.log("Gemini response received for Assessment Creator.");
        } catch (e: any) {
