...
// Find the tool data for Essay Helper
const toolData = allTools.find(tool => tool.id === 'essay-helper'); // Needs matching API /api/essay-helper
...
export default function EssayHelperPage() {
    ...
    useEffect(() => {
        if (!authIsLoading && !isAuthenticated) {
            router.push('/login?from=ai-tools/essay-helper');
            return;
        }
        if (!authIsLoading) {
    ...
    const handleSubmit = useCallback(async (formData: Record<string, any>) => {
        ...
        setResponse(null);
        console.log('Submitting Essay Helper Request:', formData);

        try {
            const apiResponse = await fetch('/api/essay-helper', {
                method: 'POST',
                headers: {
    ...
    if (!toolData) {
        return (
            <div className={styles.pageContainer}>
                <p>Error: Essay Helper tool data could not be loaded.</p>
            </div>
        );
    }
    ...
    return (
        <div className={styles.pageContainer}>
            ...
            <AiToolForm
                ...
                submitButtonText="Get Assistance"
            />
