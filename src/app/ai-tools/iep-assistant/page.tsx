...
const toolData = allTools.find(tool => tool.id === 'iep-assistant');
...
const IepAssistantPage = () => {
    ...
    const handleSubmit = useCallback(async (formData: Record<string, any>) => {
        ...
        setIsLoading(true);
        setResponse(null);
        console.log('Submitting IEP Goal Writer Request:', formData);

        try {
            const apiResponse = await fetch('/api/iep-assistant', {
                method: 'POST',
                headers: {
...
export default IepAssistantPage; 