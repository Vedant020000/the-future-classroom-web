...
const toolData = allTools.find(tool => tool.id === 'lesson-generator');
...
export default function LessonGeneratorPage() {
    ...
    useEffect(() => {
        if (!authIsLoading && !isAuthenticated) {
            router.push('/login?from=ai-tools/lesson-generator');
            return;
        }
        if (!authIsLoading) {
    ...
    const handleSubmit = useCallback(async (formData: Record<string, any>) => {
        ...
        setResponse(null);
        console.log('Submitting Lesson Generator Request:', formData);

        try {
            const apiResponse = await fetch('/api/lesson-generator', {
                method: 'POST',
                headers: {
    ...
    if (!toolData) {
        return (
            <div className={styles.pageContainer}>
                <p>Error: Lesson Generator tool data could not be loaded.</p>
            </div>
        );
    }
    ...
    return (
        <div className={styles.pageContainer}>
            ...
            <AiToolForm
                ...
                submitButtonText="Generate Lesson Plan"
            />
